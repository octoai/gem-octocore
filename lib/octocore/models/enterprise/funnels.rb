  ...
  module Cassandra
    ...
    class Funnel
      ...
      def self.from_pages(pages, opts = {})
        ...
        if ::Set.new([Octo::Cassandra::Product, Octo::Cassandra::Page]).include?(pages[0].class)
      ...
      def data(ts = Time.now.floor)
        ...
        res = Octo::Cassandra::FunnelData.where(args)
        if res.count > 0
          res.first
        elsif self.enterprise.fakedata?
          args.merge!({ value: fake_data(self.funnel.count) })
          Octo::Cassandra::FunnelData.new(args).save!
        end
      end
