  ...
  module Cassandra
    ...
    class UserPersona
      ...
      def self.fakedata(user, ts)
        ...
        res = self.where(args)
        if res.count < 1
          categories = Hash[Octo::Cassandra::Category.first(rand(1..Octo::Cassandra::Category.count)).collect do |x|
            [x.cat_text, rand(20..200)]
          end]
          tags = Hash[Octo::Cassandra::Tag.first(rand(1..Octo::Cassandra::Tag.count)).collect do |x|
